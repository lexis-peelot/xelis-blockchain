...
impl<S: Storage> Blockchain<S> {
        ...
        I: Iterator<Item = &'a Hash> + ExactSizeIterator + Clone + Send + Sync,
    {
        ...
        let (difficulty, p_new) = if version >= BlockVersion::V6 {
            ...
            difficulty::calculate_difficulty(solve_time.max(1), biggest_difficulty, p, minimum_difficulty, version)
        };
    ...
    pub async fn add_new_block_with_storage(&self, holder: StorageHolder<'_, S>, block: Block, pre_verify: PreVerifyBlock, broadcast: BroadcastOption, mining: bool) -> Result<(), BlockchainError> {
        ...
        let mut block_is_ordered = false;
        ...
        if best_tip == block_hash.as_ref() {
            ...
            block_is_ordered = full_order.contains(block_hash.as_ref());
