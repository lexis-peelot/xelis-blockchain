...
impl Mempool {
    ...
    pub async fn add_tx<'ty, S: Storage>(&mut self, storage: &S, environment: &Environment<'ty>, stable_topoheight: TopoHeight, topoheight: TopoHeight, hash: Arc<Hash>, tx: Arc<Transaction>, size: usize, block_version: BlockVersion) -> Result<(), BlockchainError> {
    ...
    pub async fn clean_up<'ty, S: Storage>(&mut self, storage: &S, environment: &Environment<'ty>, stable_topoheight: TopoHeight, topoheight: TopoHeight, block_version: BlockVersion) -> Vec<(Arc<Hash>, SortedTx)> {
